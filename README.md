...
---
# 📖 Chapter 11 균형 색인검색트리 (week11)  


### 1. 핵심 개념 및 이론 총정리 (객관식 20% 대비)

* **이진 검색 트리(BST)의 한계점:** 데이터가 정렬된 순서대로 삽입되면 트리가 한쪽으로 치우치는 편향 트리(Skewed Tree)가 되어 검색 복잡도가 $O(N)$으로 악화됩니다.
* **AVL 트리:** 각 노드의 **왼쪽 서브트리 높이와 오른쪽 서브트리 높이의 차이(균형 인수, Balance Factor)**가 항상 **1 이하(-1, 0, 1)**로 유지되도록 스스로 균형을 잡는 이진 검색 트리입니다.
* **시간 복잡도:** 삽입, 삭제, 검색 모두 최악의 경우에도 항상 **$O(\log N)$**을 보장합니다.
* **균형 무너짐의 4가지 유형과 회전 해결책 (단답형/그림 문제 단골):**
  1. **LL 상태 (Left-Left):** 왼쪽 자식의 왼쪽에 노드가 추가됨 $\rightarrow$ **우회전(Right Rotation)** 1회로 해결
  2. **RR 상태 (Right-Right):** 오른쪽 자식의 오른쪽에 노드가 추가됨 $\rightarrow$ **좌회전(Left Rotation)** 1회로 해결
  3. **LR 상태 (Left-Right):** 왼쪽 자식의 오른쪽에 노드가 추가됨 $\rightarrow$ 먼저 **좌회전 후 우회전(Left-Right 변형)**으로 해결
  4. **RL 상태 (Right-Left):** 오른쪽 자식의 왼쪽에 노드가 추가됨 $\rightarrow$ 먼저 **우회전 후 좌회전(Right-Left 변형)**으로 해결

---

### 2. AVL 트리 전체 소스코드 분석 (빠짐없이 완벽 복원)

#### ① AVLNode 클래스 정의
일반 트리 노드 구조에 균형 값을 체크하기 위한 **`height`(높이)** 변수가 추가되는 것이 핵심 특징입니다.
```python
class AVLNode:
    def __init__(self, newItem, left=None, right=None, h=1):
        ...
        self.left = left
        self.right = right
        self.height = h  # ★시험 포인트: 노드의 높이를 저장 (기본값 1)
```

### ② AVLTree 클래스 및 회전(Rotation) 알고리즘 (★출제 확률 100%)  

트리의 균형이 깨졌을 때 포인터의 손을 맞바꾸는 기본 회전 함수인 _leftRotate와 _rightRotate는 시험에 통째로 쓰라고 나올 확률이 매우 높습니다.
...
class AVLTree:
    ...
    # 유틸리티 함수: 노드의 높이 반환 (None일 경우 0 처리)
    def _height(self, tNode: AVLNode) -> int:
        if tNode is None:
            return 0
        return tNode.height

    # ★중요: 우회전 (Right Rotation) - LL 상태 해결
    def _rightRotate(self, tNode: AVLNode) -> AVLNode:
        L = tNode.left              # 1. 왼쪽 자식을 L로 지정
        LR = L.right                # 2. L의 오른쪽 자식을 보관

        L.right = tNode             # 3. L의 우측 포인터를 현재 노드로 변경 (회전)
        tNode.left = LR             # 4. 현재 노드의 좌측 포인터에 보관했던 LR 연결

        # 5. 높이 값 갱신 (자식들의 높이 중 큰 값 + 1)
        tNode.height = max(self._height(tNode.left), self._height(tNode.right)) + 1
        L.height = max(self._height(L.left), self._height(L.right)) + 1

        return L                    # 6. 새로 상위 루트가 된 L 노드를 반환

    # ★중요: 좌회전 (Left Rotation) - RR 상태 해결
    def _leftRotate(self, tNode: AVLNode) -> AVLNode:
        R = tNode.right             # 1. 오른쪽 자식을 R로 지정
        RL = R.left                 # 2. R의 왼쪽 자식을 보관

        R.left = tNode              # 3. R의 좌측 포인터를 현재 노드로 변경 (회전)
        tNode.right = RL            # 4. 현재 노드의 우측 포인터에 보관했던 RL 연결

        # 5. 높이 값 갱신
        tNode.height = max(self._height(tNode.left), self._height(tNode.right)) + 1
        R.height = max(self._height(R.left), self._height(R.right)) + 1

        return R                    # 6. 새로 상위 루트가 된 R 노드를 반환
```

### ③ 균형 맞추기 오토메이션 함수 (_balance)  

현재 노드의 좌우 높이 차이를 계산하여 LL, RR, LR, RL의 4가지 유형에 맞게 위에서 만든 회전 함수들을 콤보로 장착하는 핵심 로직입니다.

```Python
    # 균형 인수(Balance Factor) 구하기: 왼쪽 높이 - 오른쪽 높이
    def _getBalance(self, tNode: AVLNode) -> int:
        if tNode is None:
            return 0
        return self._height(tNode.left) - self._height(tNode.right)

    def _balance(self, tNode: AVLNode) -> AVLNode:
        # 현재 노드의 높이 갱신
        tNode.height = max(self._height(tNode.left), self._height(tNode.right)) + 1
        
        bf = self._getBalance(tNode) # 균형 인수 확인

        # [Case 1] LL 상태: 왼쪽이 무겁고, 새 노드가 왼쪽 자식의 왼쪽에 들어감
        if bf > 1 and self._getBalance(tNode.left) >= 0:
            return self._rightRotate(tNode)

        # [Case 2] LR 상태: 왼쪽이 무겁고, 새 노드가 왼쪽 자식의 오른쪽에 들어감
        if bf > 1 and self._getBalance(tNode.left) < 0:
            tNode.left = self._leftRotate(tNode.left) # ★시험 포인트: 자식을 먼저 좌회전
            return self._rightRotate(tNode)           # 그 다음 나를 우회전

        # [Case 3] RR 상태: 오른쪽이 무겁고, 새 노드가 오른쪽 자식의 오른쪽에 들어감
        if bf < -1 and self._getBalance(tNode.right) <= 0:
            return self._leftRotate(tNode)

        # [Case 4] RL 상태: 오른쪽이 무겁고, 새 노드가 오른쪽 자식의 왼쪽에 들어감
        if bf < -1 and self._getBalance(tNode.right) > 0:
            tNode.right = self._rightRotate(tNode.right) # ★시험 포인트: 자식을 먼저 우회전
            return self._leftRotate(tNode)               # 그 다음 나를 좌회전

        return tNode # 균형이 깨지지 않았다면 자기 자신 그대로 반환
```
### ④ AVL 트리 삽입 연산 (insert)  

데이터를 일반 이진 검색 트리 규칙대로 재귀 삽입하면서, 함수가 리턴되어 위로 거슬러 올라올 때 _balance 함수를 호출하여 무너진 밸런스를 상향식으로 바로잡습니다.

```Python
    def insert(self, newItem):
        self._root = self._insertItem(self._root, newItem)

    def _insertItem(self, tNode: AVLNode, newItem) -> AVLNode:
        if tNode is None:
            return AVLNode(newItem) # 빈 자리에 도달하면 AVL 노드 생성
        ...
        # ★시험 포인트: 노드가 삽입된 후 부모 노드로 돌아오면서 밸런스 점검 및 자동 회전
        return self._balance(tNode)
```

---

## 3. 미구현 및 과제 필수 메서드 저격 (출제 0순위)  

교재 마지막 부분에 명시되어 있거나 생략된 AVL 트리의 삭제 알고리즘의 뼈대와 탐색 연산입니다.

### ① AVL 트리 삭제 연산 뼈대 구상
삭제 연산 역시 기본 BST의 _deleteItem과 동일하게 노드를 지운 뒤, 최종 리턴 직전에 self._balance(tNode)를 한 번 거쳐서 반환하도록 코드를 수정하는 것이 원리입니다.

```Python
    def delete(self, key):
        self._root = self._deleteItem(self._root, key)

    def _deleteItem(self, tNode: AVLNode, key) -> AVLNode:
        if tNode is None:
            return None
            
        if key == tNode.item:
            tNode = self._deleteNode(tNode) # 노드 삭제 수행 (Ch10 로직)
        elif key < tNode.item:
            tNode.left = self._deleteItem(tNode.left, key)
        else:
            tNode.right = self._deleteItem(tNode.right, key)
            
        # ★시험 포인트: 삭제 후 트리가 살아있다면 무조건 밸런싱을 수행하고 반환
        if tNode is not None:
            return self._balance(tNode)
        return None
```
